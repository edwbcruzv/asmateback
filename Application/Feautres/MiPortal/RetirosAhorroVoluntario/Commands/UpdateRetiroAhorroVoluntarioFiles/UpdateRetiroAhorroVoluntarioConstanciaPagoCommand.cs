...
{
    public class UpdateRetiroAhorroVoluntarioConstanciaPagoCommand : IRequest<Response<string>>
    {
        public int Id { get; set; }
        public int AhorroVoluntarioId { get; set; }
        public IFormFile SrcDocContanciaPago { get; set; }

        public class Handler : IRequestHandler<UpdateRetiroAhorroVoluntarioConstanciaPagoCommand, Response<string>>
        {
            ...
            public async Task<Response<string>> Handle(UpdateRetiroAhorroVoluntarioConstanciaPagoCommand request, CancellationToken cancellationToken)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        if (string.IsNullOrEmpty(retiro_ahorro_voluntario.SrcDocContanciaPago))
                        {
                            retiro_ahorro_voluntario.SrcDocContanciaPago = _retiroAhorroVoluntarioService.SaveConstanciaPagoPDF(request.SrcDocContanciaPago, retiro_ahorro_voluntario.Id, request.AhorroVoluntarioId);

                        }
                        else
                        {
                            _filesManagerService.UpdateFile(request.SrcDocContanciaPago, retiro_ahorro_voluntario.SrcDocContanciaPago);
